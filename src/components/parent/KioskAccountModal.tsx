...
export function KioskAccountModal({
  ...
  const isLibre = freeAccount === true || freeAccount === undefined;
  const isRecarga = freeAccount === false;
  const hasDebt = totalDebt > 0;
  const debtExceedsLimit = totalDebt > 100;

  // Verificar si existe una solicitud de activación de Recarga pendiente
  useEffect(() => {
    if (isOpen && studentId) {
  ...
  // ── Activar modo Recarga: redirige a Pagos para depositar S/10 mínimo ──
  const handleActivateRecarga = () => {
    // Bloqueo: no puede activar Recarga si tiene deuda
    if (hasDebt) {
      toast({
        variant: 'destructive',
    ...
    // Redirige a pagos para el depósito inicial de S/10
    onClose();
    onGoToPayments();
  };

  // ── Volver a Libre: solo si no tiene deuda pendiente de activación ──
      ...
      <DialogContent className="max-w-[380px] p-0 gap-0 overflow-hidden rounded-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          {checkingPending ? (
            ...
            /* ── MODO LIBRE: botón para activar Recarga ── */
            <div className="space-y-3">
              {pendingRequest && pendingRequest.status === 'pending' ? (
                /* Solicitud de activación pendiente */
                ...
                /* Sin solicitud pendiente: puede activar Recarga */
                <div className="space-y-3">
                  {hasDebt && (
            ...
            /* ── MODO RECARGA: opciones de recarga y cambio a Libre ── */
            <div className="space-y-3">
